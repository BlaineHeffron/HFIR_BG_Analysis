...
class SpectrumData:
    ...
    def scale_hist(self, scale, scale_data=False):
        if scale_data:
            self.data *= scale
        self.hist *= scale
    ...
    def get_normalized_err(self):
        ...
        """
        norm = np.zeros((self.hist.shape[0] - 2,))
        if self.nbins != self.hist.shape[0] - 2:
            raise RuntimeError("error: nbins is not correctly set, nbins = {0}, hist shape = {1}".format(self.nbins, self.hist.shape[0]))
        for i in range(1, self.nbins + 1):
...
def numba_fill_bins(hist, old_bin_value, i, j, nbins, bin_edges, up, A1):
    """
    # new bins larger than old
    # hist is the rebinned histogram we are filling
    # old  is the old histogram's value for bin i
    # i is the bin number of the old histogram's value
    # j is the bin number of the bin we just filled
    # nbins is the number of bins
    # bin_edges is the array of bin edges
    # up is the upper bound of the old histogram
    """
    k = 1
    while j + k <= nbins and bin_edges[j + k] < up:
        hist[j + k] += old_bin_value * (bin_edges[j + k] - bin_edges[j + k - 1]) / A1
        k += 1
    if j + k == nbins + 1 and up > bin_edges[-1]:
        # rest of it goes in overflow
        hist[j + k] += old_bin_value * (up - bin_edges[-1]) / A1
    else:
        # get rest of the value in the last bin
        hist[j + k] += old_bin_value * (bin_edges[j + k] - up) / A1
                ...
                if bedge >= low:
                    if bedge >= up:
                        ...
                        if j == nbins:
                            ...
                            # new bins larger than old
                            numba_fill_bins(hist, data[i], i, j, nbins, bin_edges, up, A1)
                    break
...
def dskewgaussds(x, H, R, c, s, b):
    return -(2 * H * R * np.exp((x - c) / b - np.power((s / (sqrt(2) * b) + (x - c) / (sqrt(2) * s)), 2)) * (
                1 / (sqrt(2) * b) - (x - c) / (sqrt(2) * s ** 2))) / sqrt(np.pi)


def dskewgaussdc(x, H, R, c, s, b):
    return sqrt(2 / np.pi) * H * R * np.exp(
        (x - c) / b - np.power((s / (sqrt(2) * b) + (x - c) / (sqrt(2) * s)), 2)) / s - (
                       H * R * np.exp((x - c) / b) * erfc(s / (sqrt(2) * b) + (x - c) / (sqrt(2) * s))) / b


def dskewgaussdb(x, H, R, c, s, b):
    return (sqrt(2 / np.pi) * H * R * s * np.exp(
        (x - c) / b - np.power((s / (sqrt(2) * b) + (x - c) / (sqrt(2) * s)), 2)) - (
                        H * R * (x - c) * np.exp((x - c) / b) * erfc(s / (sqrt(2) * b) + (x - c) / (sqrt(2) * s)))) / (
                       b ** 2)
...
def ge_peak_function_jac(x, H, R, centroid, sigma, beta, A, B, C):
    dH = gauss(x, 1, R, centroid, sigma) + skewguass(x, 1, R, centroid, sigma,
                                                     beta)  # + peak_background(x, 1, step, centroid, sigma)
    dR = dgaussdR(x, H, R, centroid, sigma) + skewguass(x, H, 1, centroid, sigma, beta)
    # dstep = peak_background(x, step, 1, centroid, sigma)
    dcentroid = dgaussdc(x, H, R, centroid, sigma) + dskewgaussdc(x, H, R, centroid, sigma,
                                                                  beta)  # + dpeak_backgrounddc(x, H, step, centroid, sigma)
    dsigma = dgaussds(x, H, R, centroid, sigma) + dskewgaussds(x, H, R, centroid, sigma,
                                                               beta)  # + dpeak_backgroundds(x, H, step, centroid, sigma)
    dbeta = dskewgaussdb(x, H, R, centroid, sigma, beta)
    dA = dbackgrounddA(x)
    dB = dbackgrounddB(x)
    dC = dbackgrounddC(x)
    return np.hstack((dH.reshape(-1, 1), dR.reshape(-1, 1), dcentroid.reshape(-1, 1), dsigma.reshape(-1, 1),
                      dbeta.reshape(-1, 1), dA.reshape(-1, 1), dB.reshape(-1, 1), dC.reshape(-1, 1)))
...
class SpectrumFitter:
        ...
        for i, peak_x in enumerate(self.expected_peaks[0:-1]):
            ...
            for j, peak_x_2 in enumerate(self.expected_peaks[i + 1:]):
                ...
                if (start_ind_2 < stop_ind and start_ind_2 > start_ind) or (
                        stop_ind_2 > start_ind and stop_ind_2 < stop_ind):  # overlap detected
    ...
    def retrieve_calibration(self, user_prompt=False, tolerate_fails=False, write_to_dir=None, plot_fit=False,
                             max_sig_err=1.0, allow_undetermined=False):
        ...
        if tolerate_fails:
            ...
            if allow_undetermined:
                if n_fails > len(accept) - 1:
                    use_fits = False
            else:
                if n_fails > (len(accept) - 3):
                    use_fits = False
        else:
        ...
        if len(values) < 3:
            if allow_undetermined:
                if len(values) == 2:
                    A1 = (values[1] - values[0]) / (centroids[1] - centroids[0])
                    A0 = values[1] - A1 * centroids[1]
                elif len(values) == 1:
                    A1 = values[0] / centroids[0]
                    A0 = 0
                else:
                    return None, None
                return [A0, A1], [0,0]
