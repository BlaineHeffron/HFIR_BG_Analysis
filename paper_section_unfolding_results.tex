...
\caption{Unfolded incident gamma flux spectra for all measurement locations. The vertical axis shows the incident gamma flux in units of Hz/mm$^2$/keV. Measurement numbers correspond to the numbered positions in Figure~\ref{fig:measurement_locations}. The spectra exhibit distinct characteristics depending on the measurement location and detector orientation, with flux magnitudes varying by several orders of magnitude between positions. The energy range extends from 40~keV to 12~MeV, capturing both low-energy scattered radiation and high-energy neutron capture gamma rays.}
...
where $\Phi(E)$ is the unfolded incident differential flux spectrum (in Hz/mm$^2$/keV), $\epsilon(E)$ is the detector efficiency, and $A_{\text{eff}}(E)$ is the effective detector area as a function of energy. The CSV data files provide $\Phi(E)$ at 1~keV intervals from 40~keV to 12~MeV for all six measurement positions, enabling straightforward integration with Monte Carlo simulation packages such as Geant4 or MCNP.
